...
public abstract class JunitTests {
  ...
  public static void setUp() throws IOException {
    ...
    String configuredTestClasses = System.getProperty("qilin.test.classes");
    File testDir =
        configuredTestClasses == null
            ? new File(
                rootDir,
                "sootup.qilin" + File.separator + "target" + File.separator + "test-classes")
            : new File(configuredTestClasses);
    appPath = testDir.getCanonicalPath();
    System.out.println("APP_PATH:" + appPath);
    String configuredRefLogDir = System.getProperty("qilin.test.reflection.dir");
    File refLogDir =
        configuredRefLogDir == null
            ? new File(
                rootDir,
                "sootup.qilin"
                    + File.separator
                    ...
                    + File.separator
                    + "reflog")
            : new File(configuredRefLogDir);
    refLogPath = refLogDir.getCanonicalPath();
    String configuredJre = System.getProperty("qilin.jre");
    File jreFile =
        configuredJre == null
            ? new File(
                ".."
